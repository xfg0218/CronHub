...
public class PageViewAction extends ActionSupport {
	...
	public String findNextPageNo() {
		...
		IFindByPage<Daemon> ifinder = new IFindByPage<Daemon>() {
			...
			public List<Daemon> findByPage(int currentPage, int maxPerPage) {
				return dao.findByPage(whereSql + order_update_time_sql
						+ pageGen.generateNext(Params.TABLE_DAEMON, currentPage, maxPerPage, whereSql));// 这里的pageGen.generatePrev才是真正的生成mysql的limit造成实质分页的算法。
	...
	public String findLastPage() {
		...
		IFindByPage<Daemon> ifinder = new IFindByPage<Daemon>() {
			...
			public List<Daemon> findByPage(int currentPage, int maxPerPage) {
				return dao.findByPage(whereSql + order_update_time_sql
						+ pageGen.generateLast(Params.TABLE_DAEMON, maxPerPage, whereSql));
			}
		};
	...
	public String updateDaemonPackage() {
		...
		try {
			...
			// final String updateRemoteExecShell = String.format("wget %s -O
			// %s/%s;if [ $? -eq 0 ];then sh %s/%s %s && echo update_success ||
			// echo update_fail;else echo 'cannot download from
			// %s';fi",updateShellDownloadUrl, folder_path,
			// updateShellName,folder_path,
			// updateShellName,port,updateShellDownloadUrl);
			final String updateRemoteExecShell = String.format(
					"wget %s -O %s/%s;if [ $? -eq 0 ];then cd %s && nohup sh %s %s &  else echo 'cannot download from %s';fi",
					updateShellDownloadUrl, folder_path, updateShellName, folder_path, updateShellName, port,
					updateShellDownloadUrl);
			...
			ajaxJson.put("error", "服务器[ip:" + ip + ",端口port:" + port + "]不能通信,不能升级daemon包,请检查");
			ajaxJson.put("id", id);
